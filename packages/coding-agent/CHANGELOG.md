...
- Fixed a second cause of "The Prime Agent daemon shut down while this window was attached": launching a new window against a daemon with an outdated wire schema replaced it whenever every session was merely idle, destroying idle-but-attached windows (whose sessions, unlike detached ones, do not reload). Replacement now also refuses when any session has an attached window, including client-owned sessions invisible to the launching process. A process with a private runtime directory (`PRIME_AGENT_DAEMON_SOCKET_DIR`, as the test suite sets) also no longer discovers or kills daemons in the shared per-uid directory.
- Fixed `/refine` failing with "entry.content.replace is not a function" for the rest of a session's life: a persisted harness entry whose `content` was a list of lines instead of a string crashed every prompt build. Entry text is now normalized when harness state is loaded — a list of lines is joined, an unreadable entry is dropped rather than throwing — and the refiner's own edits accept content emitted as an array of lines instead of rejecting the edit with "requires title and content".
